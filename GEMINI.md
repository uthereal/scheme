...
*   **Guard Clauses & Early Returns:** Avoid deeply nested conditional logic. Prefer using `if condition { return/continue/break }` at the beginning of functions or loops to validate state or handle errors immediately. This flattens the logical flow and drastically improves code readability.
...
*   **Independent Closures:** Where testing independent state mutations or deeply complex setups makes table-driven arrays unwieldy, use independent `t.Run("name", func(t *testing.T) { ... })` closures instead of a loop.
