...
func migratePostgres(
  ...
  targetDbName := dbDef.GetName()
  maintenanceUri := *parsedUri
  maintenanceUri.Path = "/postgres"

  originalDbName := strings.TrimPrefix(parsedUri.Path, "/")
  if originalDbName == "" || originalDbName == "postgres" {
    parsedUri.Path = "/" + targetDbName
    dbUri = parsedUri.String()
  }

  logger.InfoContext(
    ctx,
    "Connecting to the database.",
    slog.String("target", targetDbName),
  )
  ...
  var dbNeedsCreate bool
  err = dbConn.PingContext(ctx)
  if err != nil {
    errMsg := err.Error()
    if strings.Contains(errMsg, "does not exist") ||
      strings.Contains(errMsg, "3D000") {
      dbNeedsCreate = true
    } else {
  ...
  var liveState *postgres.DatabaseState
  if dbNeedsCreate {
    liveState = &postgres.DatabaseState{
      Schemas: make(map[string]*postgres.SchemaState),
    }
  } else {
  ...
  actions, err := postgres.ComputeDiff(
    targetDbName, dbNeedsCreate, liveState, targetState,
  )
  if err != nil {
  ...
  if actions[0].ObjectType == postgres.ObjectDatabase {
    logger.InfoContext(
      ctx,
      "Target database does not exist. Provisioning now.",
      slog.String("name", targetDbName),
    )

    mConn, mErr := sql.Open("pgx", maintenanceUri.String())
    if mErr != nil {
      return fmt.Errorf(
        "failed to connect to maintenance database -> %w",
        mErr,
      )
    }

    _, mErr = mConn.ExecContext(ctx, actions[0].SQL)
    _ = mConn.Close()

    if mErr != nil {
      return fmt.Errorf(
        "failed to create target database -> %w",
        mErr,
      )
    }
    ...
    if err != nil {
      return fmt.Errorf(
        "failed to re-connect to new database -> %w",
        err,
      )
    }

    // Skip the first action (CREATE DATABASE) for Apply
    actions = actions[1:]
  }
