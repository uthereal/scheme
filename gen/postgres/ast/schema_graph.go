...
func (sg *SchemaGraph) buildModelGraph(
  ...
  // @todo doc block here with channel pass
  for _, c := range pgSchema.GetChannels() {
    _, ok := sg.NotificationChannels[c.GetName()]
    if ok {
      return fmt.Errorf("duplicate notification channel %s", c.GetName())
    }

    sg.NotificationChannels[c.GetName()] = &NotificationChannel{
      Name:    c.GetName(),
  ...
  for _, schema := range pgSchema.GetSchemas() {
    ...
    for _, table := range schema.GetTables() {
      ...
      for _, trig := range table.GetNotifyTriggers() {
        events := make([]string, 0, len(trig.GetEvents()))
        for _, ev := range trig.GetEvents() {
          events = append(events, ev.String())
        }
        sg.Triggers = append(sg.Triggers, &Trigger{
          Name:        trig.GetName(),
          Channel:     trig.GetChannel(),
          Events:      events,
          Orientation: trig.GetOrientation().String(),
          TableModel:  modelType,
        })
      }
    }
  }
