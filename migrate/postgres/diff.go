...
func (d *Differ) planFunctions() error {
  for sName, targetSchema := range d.target.Schemas {
    ...
    for _, tName := range targetFuncNames {
      ...
      _, exists := liveSchema.Functions[tName]
      if exists {
        continue
      }
      liveFunc, exists := liveSchema.Functions[pName]
      if exists {
        ...
        d.Actions = append(d.Actions, MigrationAction{
          ...
          Name:       tmpName,
          SQL: fmt.Sprintf(
            "ALTER FUNCTION %q.%q(%s) RENAME TO %q;",
            sName,
            pName,
            argStr,
            tmpName,
    ...
    for _, tName := range targetFuncNames {
      ...
      if exists {
        ...
        if exists {
          bodyMatch := strings.TrimSpace(liveFunc.Body) ==
            strings.TrimSpace(targetFunc.Body)
          sigMatch := liveFunc.Language == targetFunc.Language &&
            liveFunc.ReturnType == targetFunc.ReturnType

          if bodyMatch && sigMatch {
            namesMatch := true
            for i := range liveFunc.Arguments {
      ...
      argDefs := make([]string, 0, len(targetFunc.Arguments))
      ...
      d.Actions = append(d.Actions, MigrationAction{
        ...
        Name:       tName,
        SQL: fmt.Sprintf(
          "CREATE OR REPLACE FUNCTION %q.%q(%s) RETURNS %s LANGUAGE %s AS $$%s$$;",
          sName,
          tName,
          argStr,
          retStr,
    ...
    for _, liveName := range liveNames {
      ...
      _, ok := targetSchema.Functions[liveName]
      if !ok {
        argTypes := make([]string, 0, len(liveFunc.Arguments))
        for _, arg := range liveFunc.Arguments {
          argTypes = append(argTypes, string(arg.DataType))
    ...
    for _, tName := range targetTableNames {
      ...
      for _, fkName := range targetFKNames {
        ...
        for i, localCol := range targetFK.ColsLocal {
          ...
          targetCols = append(
            targetCols,
            fmt.Sprintf("%q", targetFK.ColsTarget[i]),
          )
        }
  ...
  for schemaName, targetSchema := range d.target.Schemas {
    ...
    for tName, targetTable := range targetSchema.Tables {
      ...
      for _, name := range targetNames {
        ...
        isIdentical := false
        if exists {
          slices.Sort(liveTrig.Events)
          slices.Sort(targetTrig.Events)
          isIdentical = (liveTrig.Orientation == targetTrig.Orientation &&
            liveTrig.Function == targetTrig.Function &&
            slices.Equal(liveTrig.Events, targetTrig.Events))
        }
        ...
        if !exists || !isIdentical {
          if exists {
            d.Actions = append(d.Actions, MigrationAction{
              Type:       ActionTypeDrop,
              ObjectType: ObjectTrigger, // Wait, need to check ObjectType constants
              Schema:     schemaName,
              Name:       name,
              SQL: fmt.Sprintf(
                "DROP TRIGGER IF EXISTS %q ON %q.%q;",
                name,
                schemaName,
                tName,
              ),
          ...
          eventsStr := strings.Join(targetTrig.Events, " OR ")
          sqlStr := fmt.Sprintf(
            "CREATE TRIGGER %q AFTER %s ON %q.%q "+
              "FOR EACH %s EXECUTE FUNCTION %q.%q();",
            name,
            eventsStr,
            schemaName,
            tName,
            targetTrig.Orientation,
            schemaName,
            targetTrig.Function,
          )
      ...
      // Drops
      liveNames := make([]string, 0, len(liveTable.Triggers))
      for name := range liveTable.Triggers {
        liveNames = append(liveNames, name)
      }
      slices.Sort(liveNames)

      for _, name := range liveNames {
        _, exists := targetTable.Triggers[name]
        if !exists {
          d.Actions = append(d.Actions, MigrationAction{
            Type:       ActionTypeDrop,
            ObjectType: ObjectTrigger,
          ...
          delete(liveTable.Triggers, name)
        }
      }
    }
  }
